...
**AgentArena** 是本仓库的 Web 端评估平台，用于在浏览器中完成完整的 Agent 评测工作流。

当团队需要评估多个 AI Agent（如不同模型、不同提示词或不同 RAG 配置）时，可通过 Web 控制台：

1. **管理数据集**：导入 JSON 或 Excel 格式的测试用例，支持版本化管理
2. **配置 Agent**：为每个待评测的 Agent 配置 HTTP 接口、认证方式、流式开关等
3. **创建评估任务**：选择数据集版本和 Agent 版本，一键发起评估
4. **执行评测**：Worker 从队列中拉取任务，调用 Agent 获取回答，再调用 LLM Judge 打分并写入数据库
5. **查看排行榜**：按任务、按运行批次查看 ELO 排名和平均分，展开查看每次问答的详细评测结果（提问、回答、优缺点、优化建议）

支持 **Persona 人设**：可根据人设描述将规范问题改写为更自然的提问，模拟真实用户表达方式。

适用对象：产品团队、算法团队、QA 及需要对 AI 回答质量做系统化评估的场景。
